...
/// Configuration for a synchronous ASCII client.
#[derive(Debug, Clone, Copy)]
pub struct AsciiClientConfig {
    /// Maximum time to wait for a response.
    pub timeout: Duration,
}

impl Default for AsciiClientConfig {
    fn default() -> Self {
        Self {
...
/// Errors that can occur while using the synchronous ASCII client.
#[derive(Debug)]
pub enum AsciiClientError {
...
impl core::fmt::Display for AsciiClientError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::Transport(e) => write!(f, "ASCII client transport error: {e}"),
            Self::Encode(e) => write!(f, "ASCII client encode error: {e:?}"),
            Self::Decode(e) => write!(f, "ASCII client decode error: {e:?}"),
            Self::Timeout => write!(f, "ASCII client timeout"),
...
impl std::error::Error for AsciiClientError {
...
impl From<TransportError> for AsciiClientError {
...
#[cfg(feature = "sync")]
#[derive(Debug)]
pub struct AsciiClient<T: Transport> {
    transport: T,
...
impl<T: Transport> AsciiClient<T> {
    ...
    pub fn new(transport: T) -> Self {
        Self::with_config(transport, AsciiClientConfig::default())
    }
    ...
    pub fn dispatch(&mut self, slave: u8, request_pdu: &[u8]) -> Result<Vec<u8>, AsciiClientError> {
        if request_pdu.is_empty() {
            return Err(AsciiClientError::InvalidResponse);
        }
        let request_function = request_pdu[0];

        let adu = AsciiAdu::new(slave, request_pdu.to_vec());
        let mut tx = [0u8; 512];
        let n = adu.encode(&mut tx).map_err(AsciiClientError::Encode)?;
        self.transport.send(&tx[..n])?;

        let mut rx = [0u8; 512];
        let m = self.transport.recv(&mut rx, self.config.timeout)?;
        if m == 0 {
            return Err(AsciiClientError::Transport(TransportError::Disconnected));
        }
        let response = AsciiAdu::decode(&rx[..m]).map_err(AsciiClientError::Decode)?;
        ...
        if response_function == request_function | ExceptionResponse::EXCEPTION_FLAG {
            let exc = ExceptionResponse::decode(&response.pdu).map_err(AsciiClientError::Decode)?;
            return Err(AsciiClientError::Exception(exc));
        }
    ...
    pub fn read_coils(
        ...
        address: u16,
        quantity: u16,
    ) -> Result<Vec<u8>, AsciiClientError> {
        let req = ReadCoilsRequest::new(address, quantity).map_err(AsciiClientError::Decode)?;
        let mut buf = [0u8; 5];
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let resp = ReadCoilsResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
        Ok(resp.coil_status)
    }
    ...
    pub fn read_discrete_inputs(
        ...
        let req =
            ReadDiscreteInputsRequest::new(address, quantity).map_err(AsciiClientError::Decode)?;
        let mut buf = [0u8; 5];
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let resp = ReadDiscreteInputsResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
    ...
    pub fn read_holding_registers(
        ...
        let req = ReadHoldingRegistersRequest::new(address, quantity)
            .map_err(AsciiClientError::Decode)?;
        let mut buf = [0u8; 5];
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let resp = ReadHoldingRegistersResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
    ...
    pub fn read_input_registers(
        ...
        let req =
            ReadInputRegistersRequest::new(address, quantity).map_err(AsciiClientError::Decode)?;
        let mut buf = [0u8; 5];
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let resp = ReadInputRegistersResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
        Ok(resp.register_values)
    }
    ...
    pub fn write_coil(
        ...
        address: u16,
        value: bool,
    ) -> Result<(), AsciiClientError> {
        let raw = if value {
            WriteSingleCoilRequest::ON
        } else {
            WriteSingleCoilRequest::OFF
        };
        let req = WriteSingleCoilRequest::new(address, raw).map_err(AsciiClientError::Decode)?;
        let mut buf = [0u8; 5];
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let _ = WriteSingleCoilResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
        Ok(())
    }
    ...
    pub fn write_register(
        ...
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let _ = WriteSingleRegisterResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
        Ok(())
    }
    ...
    pub fn write_coils(
        ...
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let _ = WriteMultipleCoilsResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
        Ok(())
    }
    ...
    pub fn write_registers(
        ...
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let _ = WriteMultipleRegistersResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
        Ok(())
    }

    /// Read the exception status byte from `slave` (FC 0x07).
    pub fn read_exception_status(&mut self, slave: u8) -> Result<u8, AsciiClientError> {
        let req = ReadExceptionStatusRequest;
        let mut buf = [0u8; 1];
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let resp = ReadExceptionStatusResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
        Ok(resp.data)
    }
    ...
    pub fn diagnostics(
        ...
        sub_function: u16,
        data: u16,
    ) -> Result<(u16, u16), AsciiClientError> {
        let req = DiagnosticsRequest::new(sub_function, data);
        let mut buf = [0u8; 5];
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let resp = DiagnosticsResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
        Ok((resp.sub_function, resp.data))
    }

    /// Read the communication event counter from `slave` (FC 0x0B).
    pub fn get_comm_event_counter(&mut self, slave: u8) -> Result<(u16, u16), AsciiClientError> {
        let req = GetCommEventCounterRequest;
        let mut buf = [0u8; 1];
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let resp = GetCommEventCounterResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
    ...
    pub fn get_comm_event_log(
        &mut self,
        slave: u8,
    ) -> Result<(u16, u16, u16, Vec<u8>), AsciiClientError> {
        let req = GetCommEventLogRequest;
        let mut buf = [0u8; 1];
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let resp = GetCommEventLogResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
        Ok((resp.status, resp.event_count, resp.message_count, resp.events))
    }

    /// Report the server ID from `slave` (FC 0x11).
    pub fn report_server_id(&mut self, slave: u8) -> Result<Vec<u8>, AsciiClientError> {
        let req = ReportServerIdRequest;
        let mut buf = [0u8; 1];
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let resp = ReportServerIdResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
        Ok(resp.data)
    }
    ...
    pub fn mask_write_register(
        ...
        and_mask: u16,
        or_mask: u16,
    ) -> Result<(u16, u16, u16), AsciiClientError> {
        let req = MaskWriteRegisterRequest::new(reference_address, and_mask, or_mask);
        let mut buf = [0u8; 7];
        let n = req.encode(&mut buf).map_err(AsciiClientError::Encode)?;
        let pdu = self.dispatch(slave, &buf[..n])?;
        let resp = MaskWriteRegisterResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
    ...
    pub fn read_write_multiple_registers(
        ...
        let pdu = self.dispatch(slave, &buf[..n])?;
        let resp =
            ReadWriteMultipleRegistersResponse::decode(&pdu).map_err(AsciiClientError::Decode)?;
        Ok(resp.register_values)
    }
    ...
    ) -> Result<(u16, Vec<u8>), AsciiClientError> {
...
#[cfg(feature = "async")]
#[derive(Debug)]
pub struct AsyncAsciiClient<T: AsyncTransport> {
    transport: T,
    config: AsciiClientConfig,
}
