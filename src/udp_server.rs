//! Synchronous UDP server listener.
//!
//! This module is available when both the `udp` and `sync` features are
//! enabled. It wraps a [`Server`](crate::server::Server) and the UDP MBAP
//! framer so that request/response ADUs can be served as datagrams over a
//! [`std::net::UdpSocket`].
